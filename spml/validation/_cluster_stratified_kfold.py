...
class ClusterStratifiedKFold(BaseCrossValidator):
    """Cluster-stratified k-fold cross-validator.

    Fits a user-supplied clustering estimator (e.g.
    :class:`sklearn.cluster.HDBSCAN`, :class:`sklearn.cluster.OPTICS`,
    :class:`sklearn.cluster.KMeans`) to the input locations, then partitions each
    ...
    def __init__(
        ...
        clusterer,
        n_splits: int = 5,
        noise: Literal["stratify", "drop", "train_only", "nearest"] = "stratify",
        noise_label: int = -1,
        random_state=None,
    ):
