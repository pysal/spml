...
def points_with_singleton(spread_points):
    ...
    nyc = numpy.array([[-74.0, 40.7]])
    return numpy.vstack([spread_points, nyc])
...
def test_all_indices_covered(spread_points):
    ...
    n = len(spread_points)
    test_counts = numpy.zeros(n, dtype=int)
    for _, te in LeaveCellOut(grid="h3", resolution=3).split(spread_points):
        test_counts[te] += 1
    numpy.testing.assert_array_equal(test_counts, 1)
...
def test_test_is_single_cell(spread_points):
    ...
    for _, te in lco.split(spread_points):
        assert len(numpy.unique(lco.cell_ids_[te])) == 1
...
def test_get_n_splits_before_split_raises(spread_points):
    lco = LeaveCellOut(grid="h3", resolution=3)
...
def test_geodataframe_reprojected(spread_points):
    ...
    gdf_wgs = geopandas.GeoDataFrame(
        geometry=[Point(lon, lat) for lon, lat in spread_points],
        crs="EPSG:4326",
    ).to_crs("EPSG:32610")
    ...
    numpy.testing.assert_array_equal(lco_proj.cell_ids_, lco_wgs.cell_ids_)
...
def test_invalid_array_shape_raises():
    with pytest.raises(ValueError):
        list(LeaveCellOut(grid="h3", resolution=3).split(numpy.ones((10, 3))))
