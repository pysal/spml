...
def spread_gdf(spread_points):
    return gpd.GeoDataFrame(
        geometry=gpd.points_from_xy(spread_points[:, 0], spread_points[:, 1]),
        crs="EPSG:4326",
    )
...
def test_correct_number_of_folds(spread_points):
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="h3", resolution=3).split(spread_points)
    )
    assert len(splits) == 3
...
def test_train_test_disjoint(spread_points):
    for tr, te in CellStratifiedKFold(n_splits=3, grid="h3", resolution=3).split(
        spread_points
    ):
...
def test_each_fold_spans_multiple_cells(spread_points):
    """With 3 clusters at res=3 and 3 folds, every test fold draws from all 3 cells."""
    ckf = CellStratifiedKFold(
        n_splits=3, grid="h3", resolution=3, shuffle=True, random_state=0
    )
    for _, te in ckf.split(spread_points):
        cells_in_test = len(np.unique(ckf.cell_ids_[te]))
...
def test_auto_resolution_finds_enough_cells(spread_points):
    ...
    assert len(np.unique(ckf.cell_ids_)) >= 3
...
def test_auto_resolution_too_many_splits_raises(spread_points):
    ...
    with pytest.raises(ValueError, match="resolution"):
        list(
            CellStratifiedKFold(n_splits=1000, grid="h3", resolution=None).split(
                spread_points
            )
        )
...
def test_geodataframe_input(spread_gdf):
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="h3", resolution=3).split(spread_gdf)
    )
    assert len(splits) == 3
...
def test_geoseries_input(spread_gdf):
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="h3", resolution=3).split(
            spread_gdf.geometry
        )
    )
    assert len(splits) == 3
...
def test_geodataframe_reprojected(spread_points):
    ...
    np.testing.assert_array_equal(ckf_utm.cell_ids_, ckf_wgs.cell_ids_)
...
def test_healpix_basic(spread_points):
    ...
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="healpix", resolution=1).split(
            spread_points
        )
    )
    assert len(splits) == 3
...
def test_s2_basic(spread_points):
    ...
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="s2", resolution=5).split(spread_points)
    )
    assert len(splits) == 3
...
def test_a5_basic(spread_points):
    ...
    splits = list(
        CellStratifiedKFold(n_splits=3, grid="a5", resolution=3).split(spread_points)
    )
    assert len(splits) == 3
